...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['data'])) {
        ...
        if (isset($data['id_evaluacion'], $data['status'])) {
            try {
                if ($data['status'] === 'terminated' && isset($data['id_responsable'])) {
                    ...
                    $stmt = $db->prepare('UPDATE evaluacion SET id_responsable = ?, status = ?, comentario = ?, evaluated_on = ? WHERE id_evaluacion = ?');
                    $result = $stmt->execute([
                        $data['id_responsable'],
                        $data['status'],
                        $data['comentario'],
        ...
        $logger->write('No data received in POST request');
        http_response_code(400);
        echo json_encode(['message' => 'No se recibieron datos en la solicitud']);
    }
} else {
